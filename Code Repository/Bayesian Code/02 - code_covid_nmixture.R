##########
# Disease Analytics (COVID19) Model: alpha = 1/p
#
#######
...
D = dat$New.Deaths
d = D # Note: d, D are identical data, used to allow two generating conditions for 'one' node
...
# Bundle data
dat.const <- list(nTimes=nTimes, n=n, d=d, D=D, r=r,a=a)  #the data used for the simulation
...
# Compile model
jm.const10 <- jags.model("northdata2.txt", dat.const, init.const,
                         n.chains=1,n.adapt=1200000)

# Posterior samples
update(jm.const10, n.iter=1200000)

ps.const1 <- coda.samples(jm.const10, pars.const, n.iter=1400000,thin=20)

summary(ps.const1)
saveRDS(ps.const1, file = "bayes_results2.RDS")
